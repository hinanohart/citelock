...
Core has no torch dependency. The local NLI backend runs on **CPU** — no GPU required.
...
5. The answer is **allowed only if every claim is entailed.** Any of the following
   triggers a **deny** — there is no path that turns an error into an allow:
   - a contradicted or unsupported claim
   - a claim with no on-topic citation
   - zero claims, too few citations
   - a backend or decomposition error

## Architecture

<div align="center">
  <img src="docs/architecture.png" alt="citelock architecture" width="840">
</div>
...
- Detects edits to the file (single-writer tamper-evidence).
- Not a distributed log, not multi-writer safe.
- Uses no `fsync` (best-effort durability).
...
and every claim decomposition are deterministic.

- The default local model is a single forward pass in eval mode with no sampling,
  so it is deterministic for a fixed model, hardware, and batch composition.
- Scores may differ in their last digits across hardware or when the same pair is
  padded inside a different batch, which the thresholds almost always absorb.
- Choosing LLM decomposition or the LLM-judge backend flips the flag to `False` —
  citelock does not claim reproducibility it cannot keep.
...
Measured runs: seed 0, Python 3.12.3, Linux x86_64 (`0.1.0a2`), default `min_relevance=0.2`.
...
This is the honest, important number. With the relevance filter **off**, the NLI
model's confident false-contradictions on off-topic passages deny 9 of 10 correct
answers. The default filter cuts that to 1 of 10 — a real, measured ~9x reduction,
but **not zero.** A residual false-deny rate remains and will be higher on harder
data; measure it on yours before trusting the gate in production.

**Contradiction-recall set** (`eval/contradiction_recall_cases.json`, 12 wrong
answers whose retrieved passages contradict them, `gold=deny`):
...
also drops a genuine contradiction that happens to be phrased without the claim's
words, so 2 of these 12 wrong answers are allowed through at the default setting;
with the filter off, all 12 are correctly denied.

The relevance filter is a **policy-controlled trade-off** — distractor robustness
(low false-deny) vs contradiction recall (low false-allow) — not a free win. Raise
`min_relevance` for the former, lower it for the latter, and measure both on your
data. (A dropped contradiction still appears in the claim's `evidence` with
...
- **Off-topic passages and false-deny.** NLI models score unrelated passages as
  ...
  relevance-scoped contradiction model is planned. Measure your own false-deny rate.
- **The relevance filter cuts both ways.** It is lexical content-word overlap,
  so a genuine contradiction phrased with no shared vocabulary is filtered out too.
  Measured cost: 2 of 12 wrong answers allowed through (false-allow 0.17) at
  `min_relevance=0.2`, versus 0 of 12 with the filter off. This is a real
  safety/usability trade-off you tune with `min_relevance`, not a free win.
- **Tokenization is whitespace + Unicode word characters.** Works for space-delimited
  scripts (Latin incl. accents, Cyrillic, Greek, …). Scripts written without spaces
  (CJK, Thai, …) are not segmented, so the relevance filter cannot judge overlap —
  use `min_relevance=0` with a backend you trust, or supply your own tokenizer.
  Stopwords are English-only.
...
- **Multi-hop claims** spanning several passages are not yet aggregated.
